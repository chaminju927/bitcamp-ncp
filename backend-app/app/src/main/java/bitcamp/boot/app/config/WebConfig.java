...
@Configuration
// - 이 클래스는 설정에 관련된 일을 하는 클래스임을 선언한다.즉 스프링 컴포넌트로 표시하는 것.
// - Sprint IoC 컨테이너는 이 클래스의 인스턴스를 자동 생성한다.
// 없으면 스프링프레임워크사용 하지 않고 일반 클래스가 된다.

// @EnableWebMvc
// - Spring 프레임워크에서 WebMVC 기능을 사용할 수 있도록 관련 객체를 준비시키라고 선언한다.
// - 웹 애플리케이션을 제작할 때 사용할 도구가 완전하게 구비되는 것이다.
// - SpringBoot의 경우 이 애노테이션을 생략해도 된다. 이미 자동화 되있으므로
// - Spring Framework를 직접 구성할때만 필요
