...
public class Exam0151 {
  ...
  public static void main(String[] args) {
    ...
    Student s2 = new Student("홍길동", 20, false); //출력 안됨!
    Student s3 = new Student("임꺽정", 21, true);
    Student s4 = new Student("유관순", 22, true);

    System.out.println(s1 == s2); //인스턴스 비교=>false

    System.out.println(s1.hashCode()); //s1,s2는 데이터가 같지만 인스턴스 다름
    ...
    set.add(s1);
    set.add(s2); // 이미 s2의 해시값과 같은 객체(s1)가 들어 있기 때문에
    // 중복을 막기 위해 s2는 저장되지 않는다.
    set.add(s3);
    set.add(s4);
