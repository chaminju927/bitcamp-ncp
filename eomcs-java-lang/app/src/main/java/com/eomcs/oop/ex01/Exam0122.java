...
public class Exam0122 {
  ...
  public static void main(String[] args) {
    ...
    Score s = createScore("홍길동", 100, 100, 100); //createScore 메서드로 생성된 인스턴스 s
    printScore(s);
  ...
  static Score createScore(String name, int kor, int eng, int math) {
    Score s = new Score();  //score클래스의 메모리 준비

    s.name = name;  //s객체 내의 name이라는 key값에 name value가 들어간다
    s.kor = kor;
    s.eng = eng;
    s.math = math;
