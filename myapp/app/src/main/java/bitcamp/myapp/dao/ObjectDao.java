...
public abstract class ObjectDao {
  ...
  public void insert(Object value) {   //다형적 변수, object 자식객체들이 들어올수 있음

    // 노드(상자)를 만들어 값을 저장한다. 노드에는 값을 담는 필드와, 다음 주소를 담는 필드가 있다.
    Node node = new Node(value);

    //리스트의 마지막 노드와 연결한다.
    //=> 만약 리스트에 노드가 없으면 현재 노드를 시작 노드 겸 마지막 노드로 설정
    if (this.tail == null) { //size == 0 , head == null 과 같음
      this.head = this.tail = node; //head와 tail 에 노드가 담긴다.
    } else {

      // => 마지막 노드의 next필드에 새 노드 주소를 담는다.
      this.tail.next = node;
  ...
  public Object[] findAll() {
    ...
    while (cursor != null) {
      ...
      //커서가 가리키는 노드의 next field에는 다음 노드 주소가 들어있다.
      // 이것을 커서에 저장한다.
      cursor = cursor.next;
    }
    //배열에 값을 담았으면 리턴한다.
  ...
  public void delete(Object value) {
    ...
    while (i < index) {
      ...
      //커서를 다음 노드로 이동시키기 전에 노드 주소를 보관한다.
      prevNode = cursor;
  ...
  public Object get(int index) {
    ...
    //현재 커서가 가리키는 노드에서 value필드 값을 꺼내 리턴한다.
    return cursor.value;
  }
}
