...
public class MemberDaoImpl implements MemberDao {
  ...
  public void insert(Member m) {
    try (Statement stmt = con.createStatement()) {
      ...
      stmt.executeUpdate(
          sql,
          Statement.RETURN_GENERATED_KEYS  //insert실행 후 자동 증가된 PK값을 리턴 받겠다고 선언
          );
      // 1. insert 후에 자동 생성된 PK값을 꺼내올 도구 준비
      try (ResultSet keyRS = stmt.getGeneratedKeys()) {

        // 2. keyRs 도구를 이용하여 서버에서 PK 값을 가져온다.
        keyRS.next();

        // 3. 가져온 PK 레코드에서 PK 값 꺼낸다.
        int autoGeneratedMemberId = keyRS.getInt(1);  //PK컬럼이 한개이므로 PK레코드에도 한개의 컬럼만 존재

        // 4. DBMS에서 insert할때 자동 생성한 member_id값을 Member객체에 리턴한다.
