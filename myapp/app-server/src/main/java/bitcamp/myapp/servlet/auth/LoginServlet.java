...
public class LoginServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if(request.getParameter("saveEmail") != null) {
      ...
      // 쿠키 보존 시간을 지정한다.
      // => 쿠키 보존 시간을 지정하지 않으면 웹 브라우저가 실행되는 동안만 유지됨
      cookie.setMaxAge(60 * 60 * 24 * 30);  //30일 동안 유지

      // 응답 헤더에 추가한다.
      response.addCookie(cookie);
    } else {
      //쿠키에 저장된 이메일을 제거하고 싶을 때 => 0초로 설정
    ...
    if (member != null) {
      ...
      // 로그인 사용자 정보를 젖아할 세션 객체 준비
      // 1. 요청 프로토콜에 세션 ID가 쿠키로 넘어왔다면,
      //   - 세션 ID에 해당하는 객체를 찾는다.
      //   1-1. 세션 객체가 유효한 경우,
      //      - 찾은 세션 객체를 리턴한다.
      //      - 응답할 때 세션ID를 쿠키로 전달하지 않는다.
      //   1-2. 세션 객체가 타임아웃되어 유효하지 않은 경우,
      //      - 2.를 수행한다
      // 2. 요청 프로토콜에 세션ID가 쿠키로 넘어오지 않았다면,
      //    - 새 세션 객체를 생성한 후 리턴한다.
      //    - 응답시 새로 생성한 세션의 ID가 쿠키로 웹브라우저에 전달될 것이다
      //
